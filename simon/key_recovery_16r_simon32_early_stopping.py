...
data_par=np.load('attack_par.npy')
Special_p=data_par[NUM]
print(Special_p[1:8])
...
def gen_challenge():
    ...
    while(flag):
        ...
        pt0=pt0&value#choose the ciphertext pairs subpatr(X_1[17]=X_1[19]==0,see Section4.1)
...
def bayesian_key_recovery_forC1(cts, net, pre_mean, pre_std,num_cand,tmp_br,ALL_NET):
    ...
    all_keys = np.zeros(num_cand * get_max_step(),dtype=np.uint16)
    all_v = np.zeros(num_cand * get_max_step())
    
    num_iter=0
    
    for i in range(get_max_step()):
        ...
        #print(i,num_iter)
        if((num_iter>=get_min_step()) and (num_iter<=get_max_step()-1)):
            #print(np.array(all_v[:num_iter*num_cand]))
            is_add=ALL_NET[num_iter-get_min_step()].predict(np.array([all_v[:num_iter*num_cand]]))
            is_add=is_add[0]
            if(is_add<0.5):
                break
    
    #print('Iter='+str(num_iter))
    return(all_keys[:num_iter*num_cand], all_v[:num_iter*num_cand])
...
def test_bayes(cts, net, net_help, m_main,s_main, m_help,  s_help,num_cand4last,cutoff1,
               num_cand4second,num_inter4second,cutoff2,
               all_net):
    


    print('Num_cand1='+str(int(num_cand4last)),end=' # ')
    print('MinStep='+str(get_min_step()),end=' # ')
    print('MaxStep='+str(get_max_step()),end=' # ')
    print('C1='+str(cutoff1))
    
    print('Num_cand2='+str(int(num_cand4second)),end=' # ')
    print('Step2='+str(int(num_inter4second)),end=' # ')
    ...
    keys = np.random.choice(2**WORD_SIZE, num_cand4last, replace=False);
    ...
    for j in range(it):
        ...
        keys, v = bayesian_key_recovery_forC1([cts[0][i], cts[1][i], cts[2][i], cts[3][i]], 
                                               net=net,pre_mean=m_main, pre_std=s_main,
                                               num_cand=num_cand4last, tmp_br=tmp_br_last,
                                               ALL_NET=all_net);
        vtmp = np.max(v);
        ...
        if (vtmp > cutoff1):
            ...
            for i2 in l2:
                ...
                keys2,v2 = bayesian_key_recovery([c0a, c1a, c0b, c1b],
                                                 num_cand=num_cand4second, num_iter=num_inter4second,
...
def get_all_neural_network_is_add():
    Min_step=get_min_step()
    ...
    for step in range(Min_step,Max_step):
        ...
        net_name=net_name='v2res_SIMON32_round='+str(num_rounds)+'_'+str(diff)+str(num_cand)+'_'+str(step)
...
def test(n):
    ...
    All_net=get_all_neural_network_is_add()
    ...
    num_cand4last=int(Special_p[1])

    num_cand4second=int(Special_p[4])
    num_inter4second=int(Special_p[5])
    

    
    pre_file_path='./key_recovery_result/'
    key_attack_path=str(num_cand4last)+'_'+'_MinStep='+str(int(Special_p[2]))+'_MaxStep='+str(int(Special_p[3]))+'c1='+str(cutoff1)+'_'+str(num_cand4second)+'_'+str(num_inter4second)+'_'+'c2='+str(cutoff2)
    ...
    for i in range(n):
        ...
        num_used,best_index = test_bayes(ct,  net=net, net_help=net_help, m_main=m_main, s_main=s_main, m_help=m_help, s_help=s_help,
                                                num_cand4last=num_cand4last,         cutoff1=cutoff1,
...
choose_plain= test(99)#99 trails.
