...
def bayesian_key_recovery(cts, net, m , s , num_cand , num_iter):
    n = len(cts[0]);
...
def test_bayes(cts,it=500,  net=net7, net_help=net6, m_main=m7, m_help=m6, s_main=s7, s_help=s6):
    n = len(cts[0]);
    ...
    for j in range(it):
        ...
        keys, scores, v = bayesian_key_recovery([cts[0][i], cts[1][i], cts[2][i], cts[3][i]], net=net,m=m_main, s=s_main,num_cand=Num_cand1, num_iter=Num_iter1 );
        vtmp = np.max(v);
        ...
        if (vtmp > cutoff1):
            ...
            for i2 in l2:
                ...
                keys2,scores2,v2 = bayesian_key_recovery([c0a, c1a, c0b, c1b],net=net_help,m=m_help,s=s_help,num_cand=Num_cand2, num_iter=Num_iter2);
                vtmp2 = np.max(v2);
                if (vtmp2 > best_val):
...
print('Gohr',num_correct/len(arr1))
